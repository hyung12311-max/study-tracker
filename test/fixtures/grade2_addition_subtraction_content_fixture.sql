-- Disposable behavior checks for Grade 2 addition/subtraction definitions and published content.
...
begin
  if (select count(*) from public.learning_skill_definitions
      where skill_code in (
        'add-without-regrouping','add-with-regrouping','subtract-without-regrouping',
        'subtract-with-regrouping','model-addition-situation','model-subtraction-situation',
        'compare-calculation-results','find-missing-number','correct-calculation-reasoning'
      )) <> 9 then
    raise exception 'grade2 addition subtraction skill definitions are incomplete';
  end if;
  ...
  if not exists (
    ...
    where version.id = 'd6d3caf7-1375-4dde-9fea-c44b5776cc04'::uuid
      and version.status = 'published'
      and unit.unit_code = 'grade2-addition-subtraction'
      and unit.sort_order = 4
  ) then
    raise exception 'grade2 addition subtraction published identity is missing';
  end if;

  if not exists (
    select 1 from public.learning_units prerequisite
    where prerequisite.unit_code = 'grade2-three-digit-numbers'
      and exists (
        select 1 from public.learning_content_versions version
        where version.unit_id = prerequisite.id and version.status = 'published'
      )
  ) then
    raise exception 'grade2 addition subtraction prerequisite is missing';
  end if;

  if (select count(*) from public.learning_stages
      where content_version_id = 'd6d3caf7-1375-4dde-9fea-c44b5776cc04'::uuid) <> 4
     or (select count(*) from public.learning_questions question
         join public.learning_stages stage on stage.id = question.stage_id
         where stage.content_version_id = 'd6d3caf7-1375-4dde-9fea-c44b5776cc04'::uuid) <> 40
     or (select count(*) from public.learning_question_options option
         ...
         join public.learning_stages stage on stage.id = question.stage_id
         where stage.content_version_id = 'd6d3caf7-1375-4dde-9fea-c44b5776cc04'::uuid) <> 160 then
    raise exception 'grade2 addition subtraction 4/40/160 structure failed';
  end if;
  ...
  if (select count(*) from public.learning_question_skills mapping
     ...
     or (select count(distinct mapping.skill_code)
         ...
         join public.learning_questions question on question.id = mapping.question_id
         join public.learning_stages stage on stage.id = question.stage_id
         where stage.content_version_id = 'd6d3caf7-1375-4dde-9fea-c44b5776cc04'::uuid) <> 9 then
    raise exception 'grade2 addition subtraction skill mapping coverage failed';
  end if;
  ...
  if pg_typeof(target_snapshot) <> 'text[]'::regtype or cardinality(target_snapshot) <> 1 then
    raise exception 'grade2 addition subtraction snapshot compatibility failed';
  end if;

  begin
    update public.learning_question_skills set is_primary = false where question_id = target_question_id;
    raise exception 'published addition subtraction mapping update was accepted';
  exception when object_not_in_prerequisite_state then null;
  end;

  begin
    delete from public.learning_question_skills where question_id = target_question_id;
    raise exception 'published addition subtraction mapping delete was accepted';
  exception when object_not_in_prerequisite_state then null;
  end;
  ...
  if has_table_privilege('anon','public.learning_question_skills','SELECT,INSERT,UPDATE,DELETE')
    ...
    raise exception 'grade2 addition subtraction mapping ACL failed';
  end if;
end
$fixture$;

select 'grade2 addition subtraction content fixture passed' as result;
