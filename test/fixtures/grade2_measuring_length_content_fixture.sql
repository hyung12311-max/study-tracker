-- Disposable behavior checks for Grade 2 measuring length definitions and published content.
...
begin
  if (select count(*) from public.learning_skill_definitions
    ...
    raise exception 'grade2 measuring length skill definitions are incomplete';
  end if;
  ...
  if not exists (
    ...
    where version.id = '29ac4fe6-847f-4c01-a402-ffce3440fad7'::uuid
      and version.status = 'published'
      and unit.unit_code = 'grade2-measuring-length'
      and unit.sort_order = 5
  ) then
    raise exception 'grade2 measuring length published identity is missing';
  end if;

  if (select count(*) from public.learning_stages
      where content_version_id = '29ac4fe6-847f-4c01-a402-ffce3440fad7'::uuid) <> 4
     or (select count(*) from public.learning_questions question
         join public.learning_stages stage on stage.id = question.stage_id
         where stage.content_version_id = '29ac4fe6-847f-4c01-a402-ffce3440fad7'::uuid) <> 40
     or (select count(*) from public.learning_question_options option
         ...
         join public.learning_stages stage on stage.id = question.stage_id
         where stage.content_version_id = '29ac4fe6-847f-4c01-a402-ffce3440fad7'::uuid) <> 160 then
    raise exception 'grade2 measuring length 4/40/160 structure failed';
  end if;
  ...
  if (select count(*) from public.learning_question_skills mapping
     ...
     or (select count(distinct mapping.skill_code)
         ...
         join public.learning_questions question on question.id = mapping.question_id
         join public.learning_stages stage on stage.id = question.stage_id
         where stage.content_version_id = '29ac4fe6-847f-4c01-a402-ffce3440fad7'::uuid) <> 8 then
    raise exception 'grade2 measuring length skill mapping coverage failed';
  end if;
  ...
  if pg_typeof(target_snapshot) <> 'text[]'::regtype or cardinality(target_snapshot) <> 1 then
    raise exception 'grade2 measuring length snapshot compatibility failed';
  end if;

  begin
    update public.learning_question_skills set is_primary = false where question_id = target_question_id;
    raise exception 'published measuring length mapping update was accepted';
  exception when object_not_in_prerequisite_state then null;
  end;

  begin
    delete from public.learning_question_skills where question_id = target_question_id;
    raise exception 'published measuring length mapping delete was accepted';
  exception when object_not_in_prerequisite_state then null;
  end;
  ...
  if has_table_privilege('anon','public.learning_question_skills','SELECT,INSERT,UPDATE,DELETE')
    ...
    raise exception 'grade2 measuring length mapping ACL failed';
  end if;
end
$fixture$;

select 'grade2 measuring length content fixture passed' as result;
