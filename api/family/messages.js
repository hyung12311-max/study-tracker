...
module.exports=async function(req,res){
 ...
 try{
  ...
  if(body.messageType==="system"){
   ...
   const row=inserted?.[0];if(row)void u.sendPush(row,null);return u.json(res,200,{ok:true,created:Boolean(row),message:row?u.safe(row):null});
  }
  ...
  const sender=(await u.supabaseFetch(`family_members?select=id,display_name,avatar_emoji,is_active&family_id=eq.${claims.family}&id=eq.${claims.sub}&limit=1`))?.[0];if(!sender?.is_active)throw u.err("Member is not active.",403);
