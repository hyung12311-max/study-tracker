...
module.exports=async function handler(req,res){
 ...
 try{
  const context=await u.authenticateActiveMember(req,{requiredRole:"parent"});
  if(req.method==="GET"){
   const row=(await u.supabaseFetch(`sticker_reward_settings?select=${fields.join(",")}&family_id=eq.${encodeURIComponent(context.familyId)}&limit=1`))?.[0];
  ...
  const rows=await u.supabaseFetch("sticker_reward_settings?on_conflict=family_id",{method:"POST",headers:{Prefer:"resolution=merge-duplicates,return=representation"},body:JSON.stringify({family_id:context.familyId,...settings,updated_at:new Date().toISOString()})});
  ...
  const failure=u.publicRouteError(e,"스티커 지급 설정을 처리하지 못했습니다.","STICKER_SETTINGS_FAILED");
