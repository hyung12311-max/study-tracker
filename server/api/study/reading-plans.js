...
function inputFrom(body) {
  ...
  const assignedMemberId = String(body.assignedMemberId || "");
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(assignedMemberId)) {
    throw u.err("담당 자녀를 확인해 주세요.", 400, "INVALID_ASSIGNED_MEMBER");
  }
  ...
  return { mode, weekdays, bookTitle, startPage, endPage, assignedMemberId };
}
...
module.exports = async function readingPlans(request, response) {
  ...
  try {
    ...
    const rows = await u.supabaseFetch("rpc/create_reading_plan_for_member", {
      method: "POST",
      body: JSON.stringify({
    ...
    if (error.supabaseCode === "42501") {
      ...
      error.message = "독서 계획에 접근할 권한이 없습니다.";
    }
    ...
    return u.json(response, error.statusCode === 403 ? 403 : error.statusCode && !error.supabaseCode ? error.statusCode : 500, {
      ok: false,
      error: error.statusCode === 403 ? error.message : error.statusCode && !error.supabaseCode ? error.message : "독서 계획을 생성하지 못했습니다.",
