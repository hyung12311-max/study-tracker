...
module.exports=async function(req,res){
 ...
 try{
  const claims=u.authenticate(req);
  if(req.method==="GET"){
  ...
  if(body.messageType==="system"){
   ...
   const sender=(await u.supabaseFetch(`family_members?select=id,display_name,role,is_active&family_id=eq.${claims.family}&id=eq.${claims.sub}&limit=1`))?.[0];
   if(!sender?.is_active||sender.role!=="child")throw u.err("Active child member is required.",403);
   const plan=(await u.supabaseFetch(`study_plans?select=id,subject,workbook,status&id=eq.${encodeURIComponent(body.relatedId)}&family_id=eq.${claims.family}&assigned_member_id=eq.${claims.sub}&limit=1`))?.[0];
   if(!plan||!["done","완료"].includes(plan.status))throw u.err("Completed study record was not found.",409);
   const reward=(await u.supabaseFetch(`sticker_history?select=sticker_count&family_id=eq.${claims.family}&member_id=eq.${claims.sub}&study_plan_id=eq.${encodeURIComponent(plan.id)}&limit=1`))?.[0],awardedStickerCount=Number(reward?.sticker_count||0);
  ...
  const inserted=await u.supabaseFetch("family_messages?on_conflict=family_id,client_message_id",{method:"POST",headers:{Prefer:"resolution=ignore-duplicates,return=representation"},body:JSON.stringify({family_id:claims.family,sender_id:claims.sub,content,client_message_id:body.clientMessageId})});
  const created=Boolean(inserted?.[0]);let row=inserted?.[0];if(!row)row=(await u.supabaseFetch(`family_messages?select=*&family_id=eq.${encodeURIComponent(claims.family)}&client_message_id=eq.${encodeURIComponent(body.clientMessageId)}&limit=1`))?.[0];if(!row)throw u.err("Message was not saved.",500);const message=u.safe({...row,sender_name:sender.display_name,sender_avatar:sender.avatar_emoji});if(created)void u.sendPush(row,{...sender,id:claims.sub});return u.json(res,200,{ok:true,message});
