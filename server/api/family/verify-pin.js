...
module.exports=async function(req,res){
 ...
 try{const body=await u.readJson(req);if(!/^[a-z0-9_-]{2,40}$/.test(body.memberKey||"")||!/^\d{4}$/.test(body.pin||""))throw u.err("Select a member and enter a 4-digit PIN.");
  ...
  return u.json(res,200,{token:u.signToken(m),realtimeToken:u.signRealtimeToken(m),expiresIn:u.FAMILY_TOKEN_TTL_SECONDS,member:{id:m.member_id,family_id:m.family_id,member_key:m.member_key,display_name:m.display_name,role:m.role,avatar_emoji:m.avatar_emoji}});
