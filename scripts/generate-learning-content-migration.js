...
const MATH_CORE_COURSE_ID = "51000000-0000-4000-8000-000000000001";
...
function diagnosticName(content) {
  return `${content.unit.slug}_${content.version.label}`.replace(/[^a-z0-9_]+/g, "_");
}

function resolveUnitSortOrder(content) {
  const makeTenOrderOffset = content.course.id === MATH_CORE_COURSE_ID && content.unit.slug !== "make-ten" ? 1 : 0;
  return content.unit.displayOrder + makeTenOrderOffset;
}
...
` : "";
  ...
  const versionExistsMessage = legacyMakeTen ? "make-ten content version already exists" : `${diagnostic} content version already exists`;
  const childExistsMessage = legacyMakeTen ? "make-ten child content identifier already exists" : `${diagnostic} child content identifier already exists`;
  ...
  if exists (
    ...
    raise exception using errcode = '23505', message = '${diagnostic} course identifier conflicts with different content';
  end if;
  ...
  if not exists (
    ...
    raise exception using errcode = 'P0002', message = '${diagnostic} existing course is missing or changed';
  end if;
  ...
  if exists (
    ...
    raise exception using errcode = '23505', message = '${diagnostic} unit identifier already exists';
  end if;

  if exists (
    select 1 from public.learning_units
    where course_id = ${quote(content.course.id)}::uuid
      and sort_order = ${unitSortOrder}
  ) then
    raise exception using errcode = '23505', message = '${diagnostic} unit sort order already exists';
    ...
    where course.id = ${quote(content.course.id)}::uuid
      ...
      and unit.sort_order = ${unitSortOrder}
  ) then
    raise exception using errcode = 'P0002', message = '${legacyMakeTen ? "make-ten" : diagnostic} shared course or unit is missing or changed';
  end if;`;
...
` : content.version.number === 1 ? `insert into public.learning_units (id, course_id, unit_code, display_title, sort_order)
values (${quote(content.unit.id)}, ${quote(content.course.id)}, ${quote(content.unit.slug)}, ${quote(content.unit.title)}, ${unitSortOrder});
` : "";
...
begin
  ...
  if exists (
    ...
    raise exception using errcode = '23505', message = '${versionExistsMessage}';
  end if;
...
${rows([...stages, ...questions, ...options].map((item) => [quote(item.id) + "::uuid"]))}
    ...
    raise exception using errcode = '23505', message = '${childExistsMessage}';
  end if;
end
$preflight$;
...
function generateVerification(content) {
  const legacyMakeTen = content.unit.slug === "make-ten";
  const unitSortOrder = resolveUnitSortOrder(content);
  const unitSortOrderCheck = legacyMakeTen
    ? "(expected.document->'unit'->>'displayOrder')::integer"
    : String(unitSortOrder);
  const diagnostic = diagnosticName(content);
  const checkPrefix = legacyMakeTen ? "make_ten" : diagnostic;
  const recommendation = content.recommendation;
  ...
  where metadata.unit_id = ${quote(content.unit.id)}::uuid` : "";
  const genericChecks = !legacyMakeTen ? `

  union all
  select 23, '${diagnostic}_question_weights_exact',
    count(*) = ${questionCount}
      and bool_and(expected.weight = 1)
      and count(actual.id) = ${questionCount},
  ...
  union all
  select 24, '${diagnostic}_pass_threshold_contract',
  ...
  union all
  select 25, '${diagnostic}_forbidden_course_absent',
    count(*) = 0,
    jsonb_build_object('count', count(*))
  from public.learning_courses course
  ...
  union all
  select 26, '${diagnostic}_answers_empty',
    count(*) = 0,
    jsonb_build_object('count', count(*))
  from public.learning_attempt_answers answer
  join public.learning_attempts attempt on attempt.id = answer.attempt_id
  where attempt.content_version_id = ${quote(content.version.id)}::uuid

  union all
  select 27, '${diagnostic}_make_ten_unit_sort_order_preserved',
    count(*) = 1,
  ...
  union all
  select 28, '${diagnostic}_course_unit_sort_orders_unique',
    count(*) = 0,
    jsonb_build_object('duplicate_orders', count(*))
  from (
    select unit.course_id, unit.sort_order
    from public.learning_units unit
    where unit.course_id = ${quote(content.course.id)}::uuid
...
with expected_content as (
  ...
  select
    ...
    question->>'explanation' as explanation,${legacyMakeTen ? "" : `
  ...
  union all
  select 2, '${checkPrefix}_unit_exact', count(*) = 1,
    jsonb_build_object('count', count(*))
  ...
  where unit.id = (expected.document->'unit'->>'id')::uuid
    ...
    and unit.sort_order = ${unitSortOrderCheck}

  union all
  select 3, '${checkPrefix}_version_published', count(*) = 1,
    jsonb_build_object('count', count(*))
  ...
  union all
  select 4, '${checkPrefix}_stages_exact',
  ...
  union all
  select 5, '${checkPrefix}_questions_exact',
  ...
  union all
  select 6, '${checkPrefix}_options_exact',
  ...
  union all
  select 7, '${checkPrefix}_structure_and_orders',
    count(*) = ${questionCount}
  ...
  union all
  select 8, '${checkPrefix}_stage_question_counts',
  ...
  union all
  select 9, '${checkPrefix}_no_user_learning_or_reward_rows',
...
from checks
union all
select 999, '${checkPrefix}_content_verification_summary', bool_and(passed),
...
` : "";
  ...
  const historyMessage = legacyMakeTen
    ? "rollback blocked: make-ten content has assignment or learning history"
    : `rollback blocked: ${diagnostic} content has assignment or learning history`;
  return `-- Pre-use rollback for the ${rollbackSubject} only.
begin;
...
module.exports = { generateMigration, generateRollback, generateVerification, resolveUnitSortOrder };
